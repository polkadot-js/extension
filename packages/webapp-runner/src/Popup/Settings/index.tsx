...
import { keyringLock, windowOpen } from "@subwallet-webapp/messaging"
      ...
      <>
        ...
        <div className={"__scroll-container"}>
          {SettingGroupItemType.map((group) => {
            return (
              <div className={"__group-container"} key={group.key}>
                {!!group.label && (
                  <div className="__group-label">{group.label}</div>
                )}
                  ...
                  {group.items.map((item) =>
                    item.isHidden ? null : (
                      <SettingItem
                        ...
                        key={item.key}
                        leftItemIcon={generateLeftIcon(
                          item.leftIconBgColor,
                          item.leftIcon
                        )}
                        name={item.title}
          ...
          <Button
            block
            icon={<Icon phosphorIcon={Lock} type="phosphor" weight={"fill"} />}
            loading={locking}
            onClick={onLock}
            schema={"secondary"}
          >
          ...
          <div className={"__version"}>SubWallet v {"EXTENSION_VERSION"}</div>
        </div>
...
export const Settings = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      height: "100%",
