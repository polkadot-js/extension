...
import ConfirmationGeneralInfo from "@subwallet-webapp/components/Confirmation/ConfirmationGeneralInfo";
import {
  changeAuthorizationBlock,
  changeAuthorizationPerSite,
...
import {
  accountCanSign,
  getSignMode,
} from "@subwallet-webapp/util/account/account";
...
function Component({
  ...
  useEffect(() => {
    if (!!authInfo?.isAllowedMap && !!authInfo?.accountAuthType) {
      const connected = Object.values(authInfo.isAllowedMap).filter(
        (s) => s
      ).length;
  ...
  const connectIconProps = useMemo<ConnectIcon>(() => {
    if (_isNotConnected) {
      return {
        linkIcon: <Icon customSize="24px" phosphorIcon={GlobeHemisphereWest} />,
    ...
    if (isBlocked) {
      return {
        linkIcon: <Icon customSize="24px" phosphorIcon={ShieldSlash} />,
  ...
  const renderContent = () => {
    if (_isNotConnected) {
      return (
        <>
          <div className={"__content-heading"}>
            {t("This is not a Web3 application")}
          </div>
    ...
    const origin = Object.entries(allowedMap).map(([address, value]) => ({
      address,
      value,
    }));
    const list = origin.filter(({ address }) =>
      accountCanSign(getSignMode(findAccountByAddress(accounts, address)))
    );
    ...
    return (
      <>
        <div className={CN("__number-of-select-text")}>
          {t("You have {{oldConnected}} accounts connected to this site", {
            replace: { oldConnected },
          })}
        </div>
...
export const ConnectWebsiteModal = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      ".ant-sw-modal-body": {
        display: "flex",
