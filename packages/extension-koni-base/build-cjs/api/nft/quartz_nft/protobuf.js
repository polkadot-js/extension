...
function convertEnumToString(value, key, NFTMeta, locale) {
  ...
  try {
    ...
    const valueJsonComment = options[value]; // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment,@typescript-eslint/no-unsafe-argument
...
const deserializeNft = (schema, buffer, locale) => {
  const root = defineMessage(schema); // Obtain the message type
