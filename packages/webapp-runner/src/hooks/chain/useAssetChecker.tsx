...
import { updateAssetSetting } from "@subwallet-webapp/messaging";
...
export default function useAssetChecker() {
  ...
  const { assetRegistry, assetSettingMap } = useSelector(
  ...
  useEffect(() => {
    if (enablingAsset && assetSettingMap[enablingAsset]?.visible) {
      ...
      const message = t("{{name}} is turned on.", {
        replace: { name: assetInfo?.symbol },
      });
  ...
  const ensureAssetEnable = useCallback(
    (assetSlug: string) => {
      ...
      if ((assetInfo && !assetSetting) || !assetSetting.visible) {
        const message = t(
          ...
          { replace: { name: assetInfo?.symbol, chainName: chainInfo?.name } }
        );
