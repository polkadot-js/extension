...
const assetRegistrySlice = createSlice({
  ...
  reducers: {
    updateAssetRegistry(
      state,
      action: PayloadAction<Record<string, _ChainAsset>>
    ) {
    ...
    updateMultiChainAssetMap(
      state,
      action: PayloadAction<Record<string, _MultiChainAsset>>
    ) {
      const { payload } = action;
    ...
    updateAssetSettingMap(
      state,
      action: PayloadAction<Record<string, AssetSetting>>
    ) {
      const { payload } = action;
