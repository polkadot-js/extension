...
export default function useGetAccountInfoByAddress(
  address: string
): AccountJson | undefined {
  const accountInfoList = useSelector(
  ...
  for (const accountInfo of accountInfoList) {
    ...
    if (
      reformatAddress(accountInfo.address, 42, isAddressEvm) ===
      reformatAddress(address, 42, isAddressEvm)
    ) {
      result = accountInfo;
      break;
    }
