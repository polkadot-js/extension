...
import useMetadata from "@subwallet-webapp/hooks/transaction/confirmation/useMetadata";
...
const renderMethod = (
  ...
  if (!args || !method) {
    return (
      <MetaInfo.Data label={t<string>("Method data")}>{data}</MetaInfo.Data>
    );
  }
...
const Component: React.FC<Props> = ({
  ...
  return (
    <MetaInfo className={className}>
      {chainInfo ? (
        <MetaInfo.Chain chain={chainInfo.slug} label={t<string>("Network")} />
      ) : (
        <MetaInfo.Default label={t<string>("GenesisHash")}>
      ...
      <MetaInfo.Account
        ...
        name={account.name}
        networkPrefix={
          chain?.ss58Format ?? chainInfo?.substrateInfo?.addressPrefix
        }
      />
      <MetaInfo.Number
      ...
      />
      <MetaInfo.Number label={t<string>("Nonce")} value={formatNumber(nonce)} />
      {!tip.isEmpty && (
        <MetaInfo.Number label={t<string>("Tip")} value={formatNumber(tip)} />
      )}
      {renderMethod(method, decoded, t)}
      <MetaInfo.Data label={t("Lifetime")}>
...
const SubstrateExtrinsic = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      ".method-container": {
