...
import AccountItem, {
  AccountItemProps,
} from "@subwallet/react-ui/es/web3-block/account-item";
...
export interface AccountItemBaseProps
  extends Omit<AccountItemProps, "avatarIdentPrefix">,
    ThemeProps {
...
const Component: React.FC<AccountItemBaseProps> = (
  props: AccountItemBaseProps
) => {
  const {
    address,
    genesisHash,
...
const AccountItemBase = styled(Component)<AccountItemBaseProps>(
  ({ theme: { token } }: AccountItemBaseProps) => {
    return {
      ".ant-account-item": {
        minHeight: 52,
        paddingTop: 0,
