...
class KoniExtension extends _Extension.default {
  ...
  accountsGetAll(id, port) {
    ...
    const subscription = _uiKeyring.default.keyringOption.optionsSubject.subscribe(options => {
  ...
  saveRecentAccountId(_ref3) {
    let {
      accountId
    } = _ref3;
    return _uiKeyring.default.saveRecent(accountId);
  }
  ...
  toggleBalancesVisibility(id, port) {
    ...
    _index.state.getSettings(value => {
      const updateValue = { ...value,
        isShowBalance: !value.isShowBalance
      };

      _index.state.setSettings(updateValue, () => {
  ...
  async subscribeSettings(id, port) {
    ...
    const balancesVisibilitySubscription = _index.state.subscribeSettingsSubject().subscribe({
      next: rs => {
        cb(rs);
      }
    });
  ...
  async accountsCreateSuriV2(_ref6) {
    let {
      genesisHash,
      isAllowed,
      name,
  ...
  async accountsForgetOverride(_ref7) {
    let {
      address
    } = _ref7;
  ...
  updateTransactionHistory(_ref14, id, port) {
    let {
      address,
      item,
      networkKey
  ...
  async validateTransfer(networkKey, token, from, to, password, value, transferAll) {
    ...
    if (token) {
      var _tokenInfo, _tokenInfo2;

      tokenInfo = await (0, _registry.getTokenInfo)(networkKey, _index.dotSamaAPIMap[networkKey].api, token);
      ...
      if ((0, _utilCrypto.isEthereumAddress)(from) && (0, _utilCrypto.isEthereumAddress)(to) && !((_tokenInfo = tokenInfo) !== null && _tokenInfo !== void 0 && _tokenInfo.isMainToken) && !((_tokenInfo2 = tokenInfo) !== null && _tokenInfo2 !== void 0 && _tokenInfo2.erc20Address)) {
        errors.push({
          code: _KoniTypes.TransferErrorCode.INVALID_TOKEN,
  ...
  async checkTransfer(_ref15) {
    ...
    if ((0, _utilCrypto.isEthereumAddress)(from) && (0, _utilCrypto.isEthereumAddress)(to)) {
      ...
      [[fee, feeSymbol], fromAccountFree, toAccountFree] = await Promise.all([(0, _transfer.estimateFee)(networkKey, fromKeyPair, to, value, !!transferAll, tokenInfo), (0, _balance.getFreeBalance)(networkKey, from, token), (0, _balance.getFreeBalance)(networkKey, to, token)]);
    }
  ...
  makeTransferCallback(address, networkKey, token, portCallback) {
    return res => {
      // !res.isFinalized to prevent duplicate action
      if (!res.isFinalized && res.txResult && res.extrinsicHash) {
        _index.state.setTransactionHistory(address, networkKey, {
          time: Date.now(),
          networkKey,
          ...
          isSuccess: res.step.valueOf() === _KoniTypes.TransferStep.SUCCESS.valueOf(),
          action: 'send',
  ...
  async makeTransfer(id, port, _ref16) {
    let {
      from,
      networkKey,
      password,
    ...
    if (errors.length) {
      setTimeout(() => {
        this.cancelSubscription(id);
      }, 500); // todo: add condition to lock KeyPair (for example: not remember password)
    ...
    if (fromKeyPair) {
      ...
      if ((0, _utilCrypto.isEthereumAddress)(from) && (0, _utilCrypto.isEthereumAddress)(to)) {
        ...
        if (tokenInfo && !tokenInfo.isMainToken && tokenInfo.erc20Address) {
          transferProm = (0, _transfer2.makeERC20Transfer)(tokenInfo.erc20Address, networkKey, from, to, privateKey, value || '0', !!transferAll, this.makeTransferCallback(from, networkKey, token, cb));
        } else {
          transferProm = (0, _transfer2.makeEVMTransfer)(networkKey, to, privateKey, value || '0', !!transferAll, this.makeTransferCallback(from, networkKey, token, cb));
        }
      } else {
        // Make transfer with Dotsama API
        transferProm = (0, _transfer.makeTransfer)(networkKey, to, fromKeyPair, value || '0', !!transferAll, tokenInfo, this.makeTransferCallback(from, networkKey, token, cb));
      }

      transferProm.then(() => {
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        console.log(`Start transfer ${transferAll ? 'all' : value} from ${from} to ${to}`); // todo: add condition to lock KeyPair
        ...
        setTimeout(() => {
          (0, _subscriptions.unsubscribe)(id);
        }, 500); // todo: add condition to lock KeyPair
  ...
  async evmNftGetTransaction(_ref17) {
    let {
      networkKey,
      params,
      recipientAddress,
  ...
  async evmNftSubmitTransaction(id, port, _ref18) {
    let {
      networkKey,
      password,
      rawTransaction,
  ...
  subscribeEvmTokenState(id, port) {
    const cb = (0, _subscriptions.createSubscription)(id, port);

    const evmTokenSubscription = _index.state.subscribeEvmToken().subscribe({
      next: rs => {
        cb(rs);
      }
    });
    ...
    return _index.state.getEvmTokenState();
  }
  ...
  upsertEvmToken(data) {
    _index.state.upsertEvmToken(data);

    return true;
  }

  deleteEvmToken(data) {
    _index.state.deleteEvmTokens(data);

    return true;
  }

  async validateEvmToken(data) {
    const evmTokenState = _index.state.getEvmTokenState();

    let isExist = false; // check exist in evmTokenState
    ...
    if (!isExist && data.type === 'erc20') {
      ...
      const chainRegistryMap = _index.state.getChainRegistryMap();

      const tokenMap = chainRegistryMap[data.chain].tokenMap;

      for (const token of Object.values(tokenMap)) {
        var _token$erc20Address;

        if ((token === null || token === void 0 ? void 0 : (_token$erc20Address = token.erc20Address) === null || _token$erc20Address === void 0 ? void 0 : _token$erc20Address.toLowerCase()) === data.smartContract.toLowerCase()) {
          isExist = true;
          break;
        }
      }
    ...
    if (data.type === 'erc721') {
      tokenContract = (0, _web.getERC721Contract)(data.chain, data.smartContract);
      const [_name, _symbol] = await Promise.all([// eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
      tokenContract.methods.name().call(), // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
      tokenContract.methods.symbol().call()]);
      name = _name;
      symbol = _symbol;
    } else {
      tokenContract = (0, _web.getERC20Contract)(data.chain, data.smartContract);
      const [_name, _decimals, _symbol] = await Promise.all([// eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
      tokenContract.methods.name().call(), // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
      tokenContract.methods.decimals().call(), // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-member-access
      tokenContract.methods.symbol().call()]);
      name = _name;
      decimals = _decimals;
      symbol = _symbol;
  ...
  async subscribeAddressFreeBalance(_ref19, id, port) {
    let {
      address,
      networkKey,
      token
    } = _ref19;
    const cb = (0, _subscriptions.createSubscription)(id, port); // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-assignment

    this.cancelSubscriptionMap[id] = await (0, _balance.subscribeFreeBalance)(networkKey, address, token, cb);
    port.onDisconnect.addListener(() => {
      this.cancelSubscription(id);
    });
    return id;
  ...
  async transferCheckReferenceCount(_ref20) {
    let {
      address,
      networkKey
    } = _ref20;
    // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-return
    return await (0, _transfer.checkReferenceCount)(networkKey, address);
  }

  async transferCheckSupporting(_ref21) {
    let {
      networkKey,
      token
    } = _ref21;
    // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-return
    return await (0, _transfer.checkSupportTransfer)(networkKey, token);
  }

  async transferGetExistentialDeposit(_ref22) {
    let {
      networkKey,
      token
    } = _ref22;
    // eslint-disable-next-line @typescript-eslint/no-unsafe-call,@typescript-eslint/no-unsafe-return
    return await (0, _transfer.getExistentialDeposit)(networkKey, token);
  ...
  async handle(id, type, request, port) {
    switch (type) {
      ...
      case 'pri(evmTokenState.upsertEvmTokenState)':
        return this.upsertEvmToken(request);

      case 'pri(evmTokenState.deleteMany)':
        return this.deleteEvmToken(request);
      ...
      case 'pri(transfer.getExistentialDeposit)':
        return await this.transferGetExistentialDeposit(request);
      ...
      case 'pri(evmTokenState.validateEvmToken)':
        return await this.validateEvmToken(request);
