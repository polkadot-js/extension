...
import { ChainLogoMap, TokenLogoMap } from "@subwallet-webapp/assets/logo";
...
const logoList = Object.entries(ChainLogoMap)
  .map(([key, value]) => {
    return {
      key,
      value,
  ...
  .concat(
    Object.entries(TokenLogoMap).map(([key, value]) => {
      return {
        key,
        value,
        type: "token",
...
function Component(props: ThemeProps) {
  ...
  const renderItem = useCallback((item: LogoItem) => {
    return (
      <div
  ...
  const searchFunction = useCallback((item: LogoItem, searchText: string) => {
    ...
    return item.key.toLowerCase().includes(searchTextLowerCase);
  }, []);
...
const DebuggerLogoPreview = styled(Component)<ThemeProps>(
  ({ theme }: ThemeProps) => ({
    ".filter-buttons": {
      display: "flex",
      marginBottom: 14,
