...
import { BN_ZERO } from "@subwallet-webapp/constants/number";
import {
  getBalanceValue,
...
import {
  ...
  CrowdloanItemType,
  CrowdloanValueInfo,
} from "@subwallet-webapp/types/crowdloan";
...
function getCrowdloanContributeList(
  ...
  chainKeys.forEach((n) => {
    ...
    const contributeValueInfo: CrowdloanContributeValueType =
      crowdloanContributeMap[n] || {
        contribute: {
          value: new BigN(0),
    ...
    if (
      !includeZeroBalance &&
      !BN_ZERO.lt(new BigN(contributeValueInfo.contribute.value))
    ) {
      return;
    }
...
function getCrowdloanContributeMap(
  ...
  crowdloanChains.forEach((key) => {
    ...
    const convertedContributeValue = getConvertedBalanceValue(
      contributeValue,
      price
    );
...
export default function useGetCrowdloanList() {
  const crowdloanMap = useSelector(
    (state: RootState) => state.crowdloan.crowdloanMap
  );
  const chainInfoMap = useSelector(
    (state: RootState) => state.chainStore.chainInfoMap
  );
  const priceMap = useSelector((state: RootState) => state.price.priceMap);
  const crowdloanChains = useMemo<string[]>(
    () => getCrowdloanChains(chainInfoMap),
    [chainInfoMap]
  );
