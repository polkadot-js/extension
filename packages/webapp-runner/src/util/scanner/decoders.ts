...
import {
  ...
  getNetworkJsonByInfo,
} from "@subwallet-webapp/util/chain/getNetworkJsonByGenesisHash";
...
export const constructDataFromBytes = (
  ...
  networkStateMap: Record<string, _ChainState>,
    ...
    switch (zerothByte) {
      ...
      case SUBSTRATE_SIGN: {
        ...
        try {
          ...
          const { account, addressLength, network } =
            findNetworkAndAccountByGenesisHash(
              networkMap,
              accounts,
              genesisHash,
              pubKeyHex
          ...
          if (network && !_isChainEnabled(networkStateMap[network.slug])) {
            console.error(
              strings.ERROR_NETWORK_INACTIVE.replace(
                "(network name)",
                network.name
              )
            );
      ...
      default:
        throw new Error(
          "Payload is not formated correctly: " + bytes.toString()
        );
    }
  } catch (e: unknown) {
    if (e instanceof Error) {
...
const findNetworkAndAccountByGenesisHash = (
  ...
  for (const forceEthereum of [false, true]) {
    network = findNetworkJsonByGenesisHash(
      networkMap,
      genesisHash,
      forceEthereum
    );
