...
import useGetChainAssetInfo from "@subwallet-webapp/hooks/screen/common/useGetChainAssetInfo";
import { deleteCustomAssets } from "@subwallet-webapp/messaging";
import { NftGalleryWrapper } from "@subwallet-webapp/Popup/Home/Nfts/component/NftGalleryWrapper";
import {
  INftCollectionDetail,
  INftItemDetail,
} from "@subwallet-webapp/Popup/Home/Nfts/utils";
...
function Component({ className = "" }: Props): React.ReactElement<Props> {
  ...
  const handleOnClickNft = useCallback(
    (state: INftItemDetail) => {
  ...
  const subHeaderButton: ButtonProps[] = [
    {
      ...
      disabled: !(
        originAssetInfo &&
        _isSmartContractToken(originAssetInfo) &&
        _isCustomAsset(originAssetInfo.slug)
      ),
    },
  ];
...
const NftCollectionDetail = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
