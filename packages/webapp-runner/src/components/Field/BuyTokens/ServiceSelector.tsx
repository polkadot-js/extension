  ...
  const renderSelected = useCallback((item: ServiceItem) => {
    return (
      <div className={"__selected-item"}>
        <Logo className={"__selected-item-logo"} network={item.key} size={24} />
  ...
  const renderItem = useCallback((item: ServiceItem, selected: boolean) => {
    return (
      <Web3Block
        ...
        leftItem={
          <Logo className={"__option-logo"} network={item.key} size={24} />
        }
...
export const ServiceSelector = styled(forwardRef(Component))<Props>(
  ({ theme: { token } }: Props) => {
    return {
