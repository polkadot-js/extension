...
import useParseSubstrateRequestPayload from "@subwallet-webapp/hooks/transaction/confirmation/useParseSubstrateRequestPayload";
import { RootState } from "@subwallet-webapp/stores";
import { ConfirmationQueueItem } from "@subwallet-webapp/stores/base/RequestState";
...
): typeof BaseTransactionConfirmation => {
    ...
    <>
      ...
      {type === "evmSendTransactionRequest" && (
        <EvmSignArea
          id={item.id}
          payload={
            item as ConfirmationDefinitions["evmSendTransactionRequest"][0]
          }
          type="evmSendTransactionRequest"
        />
      )}
...
const TransactionConfirmation = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      "--content-gap": 0,
