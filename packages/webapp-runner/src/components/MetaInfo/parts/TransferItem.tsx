...
import ChainItem from "@subwallet-webapp/components/MetaInfo/parts/ChainItem";
import { ChainInfo } from "@subwallet-webapp/types/chain";
...
const Component: React.FC<TransferInfoItem> = (props: TransferInfoItem) => {
    ...
    return (
      ...
      >
        <Avatar
          ...
          value={address}
        />
        <div className={"__account-name ml-xs"}>{name || toShort(address)}</div>
      </div>
    );
  };
  ...
  const genChainBlock = (chain: ChainInfo) => {
    return (
      ...
      >
        <Logo className={"__chain-logo"} network={chain.slug} size={24} />

        <div className={"__chain-name ml-xs"}>{chain.name}</div>
      </div>
    );
  };
  ...
  if (!recipientAddress) {
    return (
      <>
        ...
        {!!originChain && (
          <ChainItem chain={originChain.slug} label={t("Origin Chain")} />
        )}
...
const TransferItem = styled(Component)<TransferInfoItem>(
  ({ theme: { token } }: TransferInfoItem) => {
    return {};
  }
);
