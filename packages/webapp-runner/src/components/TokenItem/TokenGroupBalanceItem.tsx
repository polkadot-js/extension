...
type Props = TokenBalanceItemType &
  ThemeProps & {
...
function Component({
  ...
  return (
    <div
      className={classNames("token-group-balance-item", className, {
        ...
        rightItem={
          <>
            ...
            <div className={"__icon-wrapper"}>
              {isReady ? (
                <Icon phosphorIcon={CaretRight} size="sm" type="phosphor" />
              ) : (
                <ActivityIndicator
...
export const TokenGroupBalanceItem = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      ".ant-web3-block": {
        padding: 12,
