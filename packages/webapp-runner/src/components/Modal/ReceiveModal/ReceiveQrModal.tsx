...
const Component: React.FC<Props> = ({
  ...
  const scanExplorerAddressUrl = useMemo(() => {
    let route = "";
    const blockExplorer =
      selectedNetwork && _getBlockExplorerFromChain(chainInfo);
    ...
    if (blockExplorer) {
      ...
      return getScanExplorerAddressInfoUrl(
        selectedNetwork || "",
        formattedAddress
      );
    }
  ...
  const handleClickViewOnExplorer = useCallback(() => {
    try {
      if (scanExplorerAddressUrl) {
        ...
        void chrome.tabs
          .create({ url: scanExplorerAddressUrl, active: true })
  ...
  const onClickCopyBtn = useCallback(
    () => notify({ message: t("Copied to clipboard") }),
    [notify, t]
  );
    ...
    >
      <>
        <div className="receive-qr-code-wrapper">
          <QRCode errorLevel="H" size={264} value={formattedAddress} />
        </div>
        ...
        <div className={"receive-account-item-wrapper"}>
          <AccountItem
            ...
            leftItem={<Logo network={chainInfo?.slug} size={24} />}
            rightItem={
...
const ReceiveQrModal = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      ".items-container": {
        display: "flex",
