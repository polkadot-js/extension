...
import { createAccountHardwareMultiple } from "@subwallet-webapp/messaging"
...
const FooterIcon = <Icon phosphorIcon={Swatches} weight="fill" />
...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const networkName = useMemo(
    () =>
      selectedChain?.displayName.replace(/ network/g, "") || "Unknown network",
    [selectedChain]
  )

  const { error, getAddress, isLoading, isLocked, ledger, refresh, warning } =
    useLedger(chain)
  ...
  const onLoadMore = useCallback(
    (loading: boolean, page: number): (() => void) => {
      return () => {
        if (!loading) {
          setIsLoadMore(true)
          setPage(page + 1)
          ...
          const handler = async () => {
            ...
            const rs: Array<ImportLedgerItem | null> =
              new Array<ImportLedgerItem | null>(LIMIT_PER_PAGE).fill(null)
  ...
  const onClickItem = useCallback(
    (
      selectedAccounts: ImportLedgerItem[],
  ...
  const renderItem = useCallback(
    (
      selectedAccounts: ImportLedgerItem[]
    ): ((item: ImportLedgerItem | null, key: string) => React.ReactNode) => {
      // eslint-disable-next-line react/display-name
      return (item: ImportLedgerItem | null, key: string) => {
        if (!item) {
          return <AccountWithNameSkeleton direction="vertical" key={key} />
        }
      ...
      >
        <div className={CN("container")}>
          <div className="sub-title">
            {t(
              "Connect and unlock your Ledger, then open the DApps on your Ledger."
            )}
          </div>
          {firstStep && (
            <>
                  ...
                  <span className="ledger-info-text">
                    {t(
                      isConnected
                        ...
                        : error ||
                          (ledger ? "Waiting" : "Searching Ledger device")
                    )}
                  </span>
                  {isConnected && (
                    <Icon
...
const ConnectLedger = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
