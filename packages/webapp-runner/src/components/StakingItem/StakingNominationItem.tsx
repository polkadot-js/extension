...
import { useGetNativeTokenBasicInfo } from "@subwallet-webapp/hooks";
import { Theme, ThemeProps } from "@subwallet-webapp/types";
import { formatBalance, toShort } from "@subwallet-webapp/util";
  ...
  return (
    <div className={CN(className)}>
      <Web3Block
        ...
        leftItem={
          <SwAvatar size={40} value={nominationInfo.validatorAddress} />
        }
        middleItem={
          <>
            <div className={"middle-item__name-wrapper"}>
              <div className={"middle-item__name"}>
                {nominationInfo.validatorIdentity ||
                  toShort(nominationInfo.validatorAddress)}
              </div>
            </div>
...
const StakingNominationItem = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
