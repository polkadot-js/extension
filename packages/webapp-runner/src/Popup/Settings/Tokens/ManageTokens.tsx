...
import TokenToggleItem from "@subwallet-webapp/components/TokenItem/TokenToggleItem";
...
function Component({ className = "" }: Props): React.ReactElement<Props> {
    ...
    return (chainAsset) => {
      ...
      for (const filter of selectedFilters) {
        if (filter === FilterValue.CUSTOM) {
          ...
          if (
            assetSettingMap[chainAsset.slug] &&
            assetSettingMap[chainAsset.slug].visible
          ) {
            return true;
          }
  ...
  const subHeaderButton: ButtonProps[] = useMemo(() => {
    return [
      {
        icon: <Icon phosphorIcon={Plus} size="sm" type="phosphor" />,
        onClick: () => {
