...
export const useGetStakeData = (
  address: string,
  type: string,
  defaultChainStakingMetadata?: ChainStakingMetadata,
  defaultNominatorMetadata?: NominatorMetadata,
  slug?: string
) => {
  ...
  return useMemo(() => {
    if (slug) {
      ...
      const matchedItem = data.find(
        (item) =>
          item.staking.chain === chain &&
          item.staking.address === address &&
          item.staking.type === type
      );
    ...
    address,
    data,
    defaultChainStakingMetadata,
    defaultNominatorMetadata,
    slug,
    type,
  ]);
};
