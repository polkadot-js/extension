...
import { AssetRegistryStore } from "@subwallet-webapp/stores/types";
import { TokenGroupHookType } from "@subwallet-webapp/types/hook";
import { isTokenAvailable } from "@subwallet-webapp/util/chain/chainAndAsset";
...
export default function useTokenGroup(
  ...
  const assetSettingMap = useSelector(
    (state: RootState) => state.assetRegistry.assetSettingMap
  );
