...
import useGetChainInfoByChainId from "@subwallet-webapp/hooks/chain/useGetChainInfoByChainId";
...
const convertToBigN = (
  num: EvmSendTransactionRequest["value"]
...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const handlerRenderArg = useCallback(
    (data: EvmTransactionArg, parentName: string): JSX.Element => {
  ...
  const renderInputInfo = useCallback((): React.ReactNode => {
    ...
    return (
      <>
        ...
        <MetaInfo.Data className="arg-container" label={t("Arguments")}>
  ...
  return (
    <MetaInfo className={className}>
      {chainInfo ? (
        ...
        <MetaInfo.Default label={t<string>("Chain id")}>
          {chainId}
        </MetaInfo.Default>
      ) : null}
      <MetaInfo.Transfer
...
const EvmTransactionDetail = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
