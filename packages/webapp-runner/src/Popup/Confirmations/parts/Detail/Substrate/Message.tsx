...
const Component: React.FC<Props> = (props: Props) => {
  ...
  return (
    <MetaInfo className={CN(className)}>
      <MetaInfo.Data label={t("Raw data")}>{bytes}</MetaInfo.Data>
      <MetaInfo.Data label={t("Message")}>{message}</MetaInfo.Data>
    </MetaInfo>
  );
};

const SubstrateMessageDetail = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {};
  }
);
