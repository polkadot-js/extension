...
class KoniCron {
  ...
  init() {
    ...
    this.addCron('recoverWeb3', this.recoverWeb3Api, _constants.CRON_AUTO_RECOVER_WEB3_INTERVAL, false);

    _handlers.state.getCurrentAccount(currentAccountInfo => {
      if (currentAccountInfo) {
        this.addCron('refreshNft', this.refreshNft(currentAccountInfo.address, _handlers.state.getErc721Tokens()), _constants.CRON_REFRESH_NFT_INTERVAL);
      ...
      _handlers.state.subscribeServiceInfo_().subscribe({
        next: _ref => {
          let {
          ...
          this.addCron('refreshNft', this.refreshNft(address, customErc721Registry), _constants.CRON_REFRESH_NFT_INTERVAL);
  ...
  recoverWeb3Api() {
    ...
    for (const [key, web3] of Object.entries(_web.web3Map)) {
      web3.eth.net.isListening().catch(() => {
        console.log('web3 disconnected', key);
        (0, _web.recoverWeb3Api)(key);
      });
    }
  ...
  refreshNft(address, customErc721Registry) {
    return () => {
