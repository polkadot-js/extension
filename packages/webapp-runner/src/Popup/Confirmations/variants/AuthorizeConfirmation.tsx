...
} from "@subwallet-webapp/constants";
import {
...
import { isAccountAll, isNoAccount } from "@subwallet-webapp/util";
...
export const filterAuthorizeAccounts = (
  ...
  if (accountAuthType === "evm") {
    rs = rs.filter(
      (acc) => !isAccountAll(acc.address) && acc.type === "ethereum"
    );
...
function Component({ className, request }: Props) {
  ...
  const onConfirm = useCallback(() => {
    setLoading(true);
    const selectedAccounts = Object.keys(selectedMap).filter(
      (key) => selectedMap[key]
    );
    ...
    <>
      <div className={CN("confirmation-content", className)}>
        ...
        <div className="description">
          {visibleAccounts.length === 0
            ? t(
                "You don't have any accounts to connect. Please create a new account"
              )
        ...
        {visibleAccounts.length > 0 && (
          <>
            <Button
              ...
              onClick={onBlock}
            />
            <Button disabled={loading} onClick={onCancel} schema={"secondary"}>
              {t("Cancel")}
            </Button>
            <Button
...
const AuthorizeConfirmation = styled(Component)<Props>(
