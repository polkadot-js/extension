...
export function createSubscription<TMessageType extends MessageTypesWithSubscriptions> (id: string, getPort: () => chrome.runtime.Port): (data: SubscriptionMessageTypes[TMessageType]) => void {
  subscriptions[id] = getPort;

  return (subscription: unknown): void => {
    if (subscriptions[id]) {
      getPort().postMessage({ id, subscription });
    }
  };
}
