...
import { Avatar } from "@subwallet-webapp/components";
      ...
      <>
        ...
        <div className={CN("middle-item__info", `status-${status}`)}>
          <Icon
            ...
            phosphorIcon={
              status === UnstakingStatus.CLAIMABLE ? CheckCircle : Spinner
            }
            size="sm"
            weight="fill"
          />
...
const StakingUnstakeItem = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
