...
import AvatarGroup, {
  BaseAccountInfo,
} from "@subwallet-webapp/components/Account/Info/AvatarGroup";
...
const Component: React.FC<Props> = (props: Props) => {
  const { className, disabled, label, loading, onClick, placeholder, value } =
    props;
  ...
  return (
    ...
    >
      ...
      <div className={"select-validator-input__content-wrapper"}>
        ...
        <div className={"select-validator-input__content"}>
          {renderContent()}
        </div>
        <div className={"select-validator-input__button-wrapper"}>
          {loading ? (
            <ActivityIndicator loading={true} size={16} />
          ) : (
            <>
              <Button
                disabled={disabled}
...
const SelectValidatorInput = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
