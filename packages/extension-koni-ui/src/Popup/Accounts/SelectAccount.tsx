...
import SwAccountItem from '@subwallet/extension-koni-ui/components/Account';
...
function _SelectAccount ({ className }: Props): React.ReactElement<Props> {
  ...
  const renderItem = (item: AccountJson, _selected: boolean) => {
    return (
      <SwAccountItem name={item.name} avatarSize={24} address={item.address} genesisHash={item.genesisHash} isSelected={_selected} />
    );
  };
