...
function Component(props: Props): React.ReactElement<Props> {
  ...
  const _onChangeOption = useCallback(
  ...
  const filterModalFooter = useMemo(() => {
    return (
      <Button
        ...
        icon={<Icon phosphorIcon={FadersHorizontal} weight={"bold"} />}
  ...
  return (
    ...
    >
      <div className={"__options-container"}>
        {options.map((option) => (
          <div className={"__option-item"} key={option.value}>
            <Checkbox
...
export const FilterModal = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      ".ant-sw-modal-body": {
