...
export function getEthereumChains(
...
export const defaultRecoded: Recoded = {
  account: null,
  formatted: null,
  prefix: 42,
  isEthereum: false,
...
export function recodeAddress(
  ...
  const account =
    ...
    findSubstrateAccount(accounts, publicKey);
  const prefix =
...
export function getLogoByNetworkKey(
  networkKey: string,
  defaultLogo = "default"
): string {
  return (
    ChainLogoMap[networkKey] ||
    ChainLogoMap[defaultLogo] ||
    ChainLogoMap.default
  );
}
...
export function isSupportScanExplorer(networkKey: string): boolean {
  return (
    Object.keys(evmBlockExplorer).includes(networkKey) ||
    isSupportSubscan(networkKey)
  );
}

export function getScanExplorerTransactionHistoryUrl(
  networkKey: string,
  hash: string,
  useSubscan?: boolean
): string {
...
export function getScanExplorerAddressInfoUrl(
  networkKey: string,
  address: string
): string {
  if (Object.keys(evmBlockExplorer).indexOf(networkKey) > -1) {
...
export const detectThemeAvatar = (address?: string) =>
  isEthereumAddress(address) ? "ethereum" : "polkadot";

export { toAddress } from "./account/toAddress";
export { openInNewTab } from "@subwallet-webapp/util/common/browser";
