  ...
  return (
    <div className={CN(className, "__row -type-chain")}>
      ...
      <div className={"__col -to-right"}>
        ...
        >
          <Logo className={"__chain-logo"} network={chain} size={24} />

          <div className={"__chain-name ml-xs"}>{chainInfo?.name}</div>
        </div>
      </div>
    </div>
  );
...
const ChainItem = styled(Component)<ChainInfoItem>(
  ({ theme: { token } }: ChainInfoItem) => {
    return {};
  }
);
