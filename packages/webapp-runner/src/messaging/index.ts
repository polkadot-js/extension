...
import { _getKnownHashes, _getKnownNetworks } from "@subwallet-webapp/util/chain/defaultChains"
...
export function lazySendMessage<
...
export function lazySubscribeMessage<
  TMessageType extends MessageTypesWithSubscriptions
>(
  message: TMessageType,
  request: RequestTypes[TMessageType],
...
export async function checkPublicAndPrivateKey(
  ...
  return sendMessage("pri(accounts.checkPublicAndSecretKey)", {
    publicKey,
    secretKey,
  })
}
...
export async function approveSignPasswordV2(
  request: RequestSigningApprovePasswordV2
): Promise<boolean> {
...
export async function createAccountHardwareV2(
  request: RequestAccountCreateHardwareV2
): Promise<boolean> {
  return sendMessage("pri(accounts.create.hardwareV2)", request)
}

export async function createAccountHardwareMultiple(
  request: RequestAccountCreateHardwareMultiple
): Promise<boolean> {
...
export async function createAccountSuriV2(
  request: RequestAccountCreateSuriV2
...
export async function getMetadata(
  ...
  if (def) {
    ...
    const chain = parsedChains.find(
      (chain) => chain.genesisHash === genesisHash
    )
...
export async function subscribeAccountsInputAddress(
  cb: (data: OptionInputAddress) => void
): Promise<string> {
  return sendMessage("pri(accounts.subscribeAccountsInputAddress)", {}, cb)
}

export async function saveRecentAccountId(
  accountId: string
): Promise<SingleAddress> {
...
export async function changeAuthorizationPerSite(
  request: RequestAuthorizationPerSite
): Promise<boolean> {
  return sendMessage("pri(authorize.changeSitePerSite)", request)
}

export async function changeAuthorizationBlock(
  request: RequestAuthorizationBlock
): Promise<boolean> {
...
export async function updateChainActiveState(
  chain: string,
  active: boolean
): Promise<boolean> {
  if (active) {
...
): Promise<_ValidateCustomAssetResponse> {
...
export async function updateAssetSetting(
  data: AssetSettingUpdateReq
): Promise<boolean> {
...
export async function validateCustomChain(
  ...
  return sendMessage("pri(chainService.validateCustomChain)", {
    provider,
    existedChainSlug,
  })
}
...
export async function transferCheckReferenceCount(
  request: RequestTransferCheckReferenceCount
): Promise<boolean> {
...
export async function transferGetExistentialDeposit(
  request: RequestTransferExistentialDeposit
): Promise<string> {
...
export async function accountIsLocked(
  address: string
): Promise<ResponseAccountIsLocked> {
  return sendMessage("pri(account.isLocked)", { address })
}

export async function qrSignSubstrate(
  request: RequestQrSignSubstrate
): Promise<ResponseQrSignSubstrate> {
  return sendMessage("pri(qr.sign.substrate)", request)
}

export async function qrSignEvm(
  request: RequestQrSignEvm
): Promise<ResponseQrSignEvm> {
...
export async function getAccountMeta(
  request: RequestAccountMeta
): Promise<ResponseAccountMeta> {
...
export async function getBondingOptions(
  ...
  return sendMessage("pri(bonding.getBondingOptions)", {
    chain: networkKey,
    type,
  })
}

export async function getNominationPoolOptions(
  chain: string
...
export async function submitStakeClaimReward(
  request: RequestStakeClaimReward
): Promise<SWTransactionResponse> {
...
export async function subscribeAuthUrl(
  callback: (data: AuthUrls) => void
): Promise<AuthUrls> {
...
// Keyring state
export async function keyringStateSubscribe(
  cb: (value: KeyringState) => void
...
export async function keyringMigrateMasterPassword(
  request: RequestMigratePassword
): Promise<ResponseMigratePassword> {
  return sendMessage("pri(keyring.migrate)", request)
}

export async function keyringUnlock(
  request: RequestUnlockKeyring
): Promise<ResponseUnlockKeyring> {
...
/// Derive
export async function validateDerivePathV2(
  request: RequestDeriveValidateV2
): Promise<ResponseDeriveValidateV2> {
  return sendMessage("pri(derivation.validateV2)", request)
}

export async function getListDeriveAccounts(
  request: RequestGetDeriveAccounts
): Promise<ResponseGetDeriveAccounts> {
  return sendMessage("pri(derivation.getList)", request)
}

export async function deriveMultiple(
  request: RequestDeriveCreateMultiple
): Promise<boolean> {
  return sendMessage("pri(derivation.create.multiple)", request)
}

export async function deriveAccountV3(
  request: RequestDeriveCreateV3
): Promise<boolean> {
  return sendMessage("pri(derivation.createV3)", request)
}

export async function getTransaction(
  request: RequestGetTransaction
): Promise<SWTransactionResult> {
