...
import { IGNORE_GET_SUBSTRATE_BALANCE_LIST, MOONBEAM_REFRESH_BALANCE_INTERVAL } from '@polkadot/extension-koni-base/constants';
...
export function subscribeBalance (addresses: string[], dotSamaAPIMap: Record<string, ApiProps>, callback: (networkKey: string, rs: BalanceItem) => void) {
  ...
  return Object.entries(dotSamaAPIMap).map(async ([networkKey, apiProps]) => {
    ...
    if (!useAddresses || useAddresses.length === 0 || IGNORE_GET_SUBSTRATE_BALANCE_LIST.indexOf(networkKey) > -1) {
