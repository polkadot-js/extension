...
function Component({ className = "" }: Props): React.ReactElement<Props> {
  ...
  const contractAddressInfo = useCallback(() => {
    ...
    return (
      <span>{`${contractAddress.slice(0, 10)}...${contractAddress.slice(
        -10
      )}`}</span>
    );
  }, [tokenInfo]);
  ...
  const onChangePriceId = useCallback(
        ...
        <div className={"token_detail__container"}>
          <div className={"token_detail__header_container"}>
            <div className={"token_detail__header_icon_wrapper"}>
              <Logo size={112} token={tokenInfo.symbol.toLowerCase()} />
            </div>
          ...
          <div className={"token_detail__content_container"}>
            ...
            <Field
              ...
              prefix={<Logo network={originChainInfo.slug} size={20} />}
            />
