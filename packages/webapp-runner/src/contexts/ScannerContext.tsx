...
import {
  ...
  Transaction,
} from "@subwallet/extension-base/utils/eth";
import { SCANNER_QR_STEP } from "@subwallet-webapp/constants/qr";
import {
  parseEVMTransaction,
...
import {
  ...
  TxQRInfo,
} from "@subwallet-webapp/types/scanner";
import { findAccountByAddress } from "@subwallet-webapp/util/account/account";
import { getNetworkJsonByInfo } from "@subwallet-webapp/util/chain/getNetworkJsonByGenesisHash";
import {
  constructDataFromBytes,
  encodeNumber,
} from "@subwallet-webapp/util/scanner/decoders";
import {
  isEthereumCompletedParsedData,
  isSubstrateMessageParsedData,
} from "@subwallet-webapp/util/scanner/sign";
...
export function ScannerContextProvider({
  children,
}: ScannerContextProviderProps): React.ReactElement {
  ...
  const _setTXRequest = useCallback(
    (
      ...
      if (!sender) {
        throw new Error(
          "Account has not been imported into this device. Please import an account and try again."
        );
      }
      ...
      setState({
        ...qrInfo,
        rawPayload: (txRequest as SubstrateTransactionParsedData)?.data
          .rawPayload,
  ...
  const _setDataToSign = useCallback(
    (
      ...
      const genesisHash = (signRequest as SubstrateMessageParsedData).data
        .genesisHash;
      let message = "";
      let isHash = false;
  ...
  const signDataLegacy = useCallback(async (): Promise<void> => {
    ...
    const sender =
      !!senderAddress && findAccountByAddress(accounts, senderAddress);
    const info: undefined | number | string = isEthereumStructure
      ? evmChainId
      : genesisHash;
    ...
    if (!_isChainEnabled(senderNetworkState)) {
      throw new Error(
        `Inactive network. Please activate ${senderNetwork.name?.replace(
          " Relay Chain",
          ""
    ...
    const parseTransaction = async (): Promise<
      ResponseQrParseRLP | ResponseParseTransactionSubstrate | null
    > => {
      if (type === "message") {
        return null;
      } else {
        if (!isEthereumStructure) {
          if (genesisHash && rawPayload) {
            const _rawPayload = isString(rawPayload)
              ? rawPayload
              : u8aToHex(rawPayload);

            return parseSubstrateTransaction({
              data: _rawPayload,
              networkKey: senderNetwork.slug,
            });
          } else {
          ...
          if (dataToSign) {
            const _raw = isString(dataToSign)
              ? dataToSign
              : u8aToHex(dataToSign);
    ...
    const [signedData, parsedTx] = await Promise.all([
      signData(),
      parseTransaction(),
    ]);
