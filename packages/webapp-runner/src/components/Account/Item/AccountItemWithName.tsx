...
import AccountItemBase, {
  AccountItemBaseProps,
} from "@subwallet-webapp/components/Account/Item/AccountItemBase";
import { isAccountAll, toShort } from "@subwallet-webapp/util";
      ...
      middleItem={
        ...
        >
          <div className={"account-item-name"}>
            {isAll ? t("All accounts") : accountName}
          </div>
          {!isAll && (
...
const AccountItemWithName = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
      ".account-item-content-wrapper": {
