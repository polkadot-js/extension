...
function parseProviders(
  newProvider: string,
  existingProviders: Record<string, string>
) {
  let count = 0
...
function Component({ className = "" }: Props): React.ReactElement<Props> {
  ...
  const onSubmit = useCallback(() => {
    ...
    const { newProviderKey, newProviders } = parseProviders(
      newProvider.replace(/ /g, ""),
      chainInfo.providers
    )
    ...
    >
      ...
      >
        <div className={"add_provider__container"}>
          ...
          <Form form={form} initialValues={formInitValues()}>
