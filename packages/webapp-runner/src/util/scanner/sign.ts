...
export const unwrapMessage = (message: string) => {
  ...
  unwrapped =
    unwrapped.substr(0, prefix.length) === prefix
      ? unwrapped.substr(prefix.length, unwrapped.length)
      : unwrapped;
  unwrapped =
...
export function isEthereumCompletedParsedData(
  ...
  return (
    (parsedData as SubstrateCompletedParsedData).data.genesisHash === undefined
  );
}
