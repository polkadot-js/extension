...
const renderItem = (item: UnstakingInfo, isSelected: boolean) => (
  <StakingUnstakeItem isSelected={isSelected} unstakingInfo={item} />
);
      ...
      return (
        ...
        >
          <Icon
            ...
            phosphorIcon={
              item.status === UnstakingStatus.CLAIMABLE ? CheckCircle : Spinner
            }
            size="sm"
            weight="fill"
          />
...
const CancelUnstakeSelector = styled(forwardRef(Component))<Props>(
  ({ theme: { token } }: Props) => {
    return {
