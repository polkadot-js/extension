...
const Component: React.FC<Props> = (props: Props) => {
  ...
  const _onSelect = useCallback(() => {
    onClick &&
      onClick(getValidatorKey(validatorInfo.address, validatorInfo.identity));
      ...
      <Web3Block
        ...
        leftItem={
          <SwAvatar
            ...
            theme={
              isEthereumAddress(validatorInfo.address) ? "ethereum" : "polkadot"
            }
          ...
          <>
            <div className={"middle-item__name-wrapper"}>
              <div className={"middle-item__name"}>
                {validatorInfo.identity || toShort(validatorInfo.address)}
              </div>
              {isNominated && (
                <Icon
            ...
            <div className={"middle-item__info"}>
              ...
              -<span className={"middle-item__apy"}>{t(`APY: ${apy}%`)}</span>
            </div>
          </>
        }
        rightItem={
          <>
            {(showUnSelectedIcon || isSelected) && (
              <Icon
                ...
                iconColor={
                  isSelected ? token.colorSuccess : token.colorTextLight4
                }
                phosphorIcon={CheckCircle}
                size={"sm"}
                weight={"fill"}
...
const StakingValidatorItem = styled(Component)<Props>(
  ({ theme: { token } }: Props) => {
    return {
