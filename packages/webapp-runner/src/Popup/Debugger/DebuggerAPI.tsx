...
import { subscribeMessage } from "@subwallet-webapp/messaging";
...
const Component = ({ className }: ComponentProps) => {
    ...
    formRef.current
      ...
      .then(() => {
        ...
        const subscription = subscribeMessage(
          formValues?.api as MessageTypesWithSubscriptions,
          request,
          callback,
          callback
        );
  ...
  return (
    <div className={className}>
      ...
      >
        <Form.Item name="api">
          <Select className="input" showSearch>
            {API_LIST.map((item) => (
        ...
        >
          <Input.TextArea label={"Payload"} rows={6} />
        </Form.Item>
        <Form.Item>
        ...
        <Form.Item>
          <Input.TextArea label={"Response"} rows={6} value={response} />
        </Form.Item>
      </Form>
    </div>
  );
...
export const DebuggerAPI = styled(Component)<ComponentProps>(
