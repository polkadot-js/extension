...
import { getBondingExtrinsic, getBondingTxInfo, getChainBondingBasics, getUnbondingExtrinsic, getUnbondingTxInfo, getUnlockingInfo, getValidatorsInfo, getWithdrawalTxInfo } from '@subwallet/extension-koni-base/api/bonding';
import { getWithdrawalExtrinsic } from '@subwallet/extension-koni-base/api/bonding/relayChain';
          ...
          if (result.status.isInBlock || result.status.isFinalized) {
            result.events
              ...
              .forEach(({ event: { method } }): void => {
                ...
                if (method === 'ExtrinsicFailed') {
                  console.log('ExtrinsicFailed', method);
                  txState.status = false;
                  callback(txState);
                } else if (method === 'ExtrinsicSuccess') {
                  console.log('ExtrinsicSuccess', method);
                  txState.status = true;
                  callback(txState);
                }
              });
