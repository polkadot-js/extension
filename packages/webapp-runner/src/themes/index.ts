...
import defaultImagePlaceholder from "@subwallet-webapp/assets/default-image-placeholder.png"
import ChainLogoMap, {
  IconMap,
  TokenLogoMap,
} from "@subwallet-webapp/assets/logo"
import subWalletLogo from "@subwallet-webapp/assets/sub-wallet-logo.svg"
import SwLogosMap from "@subwallet-webapp/assets/subwallet"
import { theme as SwReactUI } from "@subwallet/react-ui"
import {
  ThemeConfig as _ThemeConfig,
  Web3LogoMap,
} from "@subwallet/react-ui/es/config-provider/context"
import {
...
function genDefaultExtraTokens(token: AliasToken): ExtraToken {
  return {
    ...
    tokensScreenSuccessBackgroundColor:
      "linear-gradient(180deg, rgba(76, 234, 172, 0.1) 16.47%, rgba(217, 217, 217, 0) 94.17%)",
    tokensScreenDangerBackgroundColor:
      "linear-gradient(180deg, rgba(234, 76, 76, 0.1) 16.47%, rgba(217, 217, 217, 0) 94.17%)",
    tokensScreenInfoBackgroundColor:
...
SW_THEME_CONFIGS[ThemeNames.SUBSPACE] = {
  ...SW_THEME_CONFIGS[ThemeNames.LIGHT],
}

export function generateTheme(
  { customTokens, generateExtraTokens, id, logoMap, name }: SwThemeConfig,
  token: GlobalToken
): Theme {
  return {
    id,
